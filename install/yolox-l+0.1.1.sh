...
# yolox-l 模型下载脚本 v0.1.1  
# YOLOX-L文档布局检测模型(大型)，最高精度但计算较慢
# 可单独执行此脚本完成模型下载

set -e

echo "=== yolox-l 模型下载脚本 v0.1.1 ==="
echo "📄 正在下载YOLOX-L文档布局检测模型..."
echo ""
...
# 创建模型下载Python脚本
cat > /tmp/download_yolox_l.py << 'EOF'
import os
import sys
import requests
...
MODEL_DESC = "YOLOX-L文档布局检测模型(大型)"
MODEL_SIZE = "~207MB"
MODEL_URL = "https://github.com/Megvii-BaseDetection/YOLOX/releases/download/0.1.1rc0/yolox_l.onnx"
...
"""
    ...
    config_path = os.path.join(cache_dir, "yolox_l_config.txt")
...
print("⚠️  注意: 这是一个大型模型(207MB)，下载可能需要较长时间")
print("⚠️  请确保网络连接稳定，并有足够的磁盘空间")
...
# 执行下载
echo "🚀 开始执行模型下载..."
python3 /tmp/download_yolox_l.py

# 清理临时文件
rm -f /tmp/download_yolox_l.py

echo ""
echo "✅ yolox-l 模型下载脚本执行完成！"
